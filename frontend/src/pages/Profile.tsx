              ...
              <TabsContent value="settings" className="mt-0">
                <div className="max-w-2xl space-y-8">
                    ...
                    <div className="rounded-2xl border border-border/20 bg-background/30 divide-y divide-border/20">
                      <div className="flex items-center justify-between px-5 py-4">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">Always enabled for cinematic experience</p>
                        </div>
                        <Switch defaultChecked disabled />
                      </div>
                      <div className="flex items-center justify-between px-5 py-4">
                        <div>
                          <p className="text-sm font-medium">Dynamic Theme Adaptation</p>
                          <p className="text-xs text-muted-foreground mt-0.5">Let chatbot keywords shift visual theme</p>
                        </div>
                  ...
                  <div>
                    ...
                    <div className="rounded-2xl border border-border/20 bg-background/30 divide-y divide-border/20">
                      {[
                        { label: 'New Films', desc: 'Get notified when new films are uploaded', on: true },
                        { label: 'New Assets', desc: 'Get notified when new 3D assets are listed', on: true },
                        { label: 'Marketing', desc: 'Promotional offers and special announcements', on: false },
                      ].map((item) => (
                        <div key={item.label} className="flex items-center justify-between px-5 py-4">
                          <div>
                            <p className="text-sm font-medium">{item.label}</p>
                    ...
                    <div className="rounded-2xl border border-border/20 bg-background/30 divide-y divide-border/20">
                      <div className="flex items-center justify-between px-5 py-4">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">Background audio in hero sections</p>
                        </div>
                        <Switch />
                      </div>
                      <div className="flex items-center justify-between px-5 py-4">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">Hover to preview film thumbnails</p>
                        </div>
                        <Switch defaultChecked />
                      </div>
                      <div className="flex items-center justify-between px-5 py-4">
                        <div>
                          <p className="text-sm font-medium">High Performance 3D</p>
                          <p className="text-xs text-muted-foreground mt-0.5">Use GPU-accelerated 3D rendering</p>
                        </div>
                        <Switch defaultChecked />
                      </div>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-5">
                      ...
                      <h3 className="text-base font-semibold">Saved Interests</h3>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {['Sci-Fi', 'AI', '3D Assets', 'Cinematic', 'Technology', 'Horror', 'Drama', 'Art House'].map(tag => (
                        <Badge
                          key={tag}
                          variant="outline"
                          className="cursor-pointer border-border/30 hover:border-primary/40 hover:bg-primary/5 transition-colors text-muted-foreground hover:text-foreground"
                        >
                          {tag}
                        </Badge>
                      ))}
                  ...
                  {/* Chatbot Theme Behavior */}
                  <div>
                    <div className="flex items-center gap-2 mb-5">
                      <Bot className="w-4 h-4 text-accent" />
                      <h3 className="text-base font-semibold">Chatbot Theme Behavior</h3>
                    </div>
                    <div className="rounded-2xl border border-border/20 bg-background/30 divide-y divide-border/20">
                      {[
                        { label: 'Keyword-based Theme Shift', desc: 'Change site theme based on chat topics', on: true },
                        { label: 'Smooth Transitions', desc: 'Animate theme changes over 800ms', on: true },
                        { label: 'Show Theme Hint', desc: 'Display a badge when theme changes', on: false },
                      ].map((item) => (
                        <div key={item.label} className="flex items-center justify-between px-5 py-4">
                          <div>
                            <p className="text-sm font-medium">{item.label}</p>
